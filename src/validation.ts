...
export function financialRequiredErrors(state: Pick<ReportState, 'incident' | 'transaction'>): string[] {
  const errors: string[] = []
  if (!state.transaction.merchantName) errors.push('Bank / wallet / merchant name is required for this complaint type.')
  ...
  if (!state.transaction.transactionDate) errors.push('Transaction date is required for this complaint type.')
